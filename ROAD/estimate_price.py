...
### 計算價格        (總淨負載、充電功率、時間電價、慢充快充、入場時間、出場時間、變壓器功率、時間段大小)
def estimate_price(total_load, p_charge, tou, ch_type, time_in, time_out, p_tr, time_scale=5):
    ...
    for i in range(len(total_load)):
        if p_charge[i] != 0 :
            ...
            total_price_of_ch += (math.pow(percent_of_grid,3)*0.4 + 0.9)*p_charge[i]*t_time
    ...
    return unit_price_of_ch, total_price_of_space
