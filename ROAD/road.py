...
class road:  #沒有新車加入
            ...
            with open('./ROAD/cpos_parameter.csv', 'r', encoding='utf-8', errors='ignore', newline='') as file:
                ...
                for row in csv_reader:
                    if(row[1] == '2'):
                        ...
                        self.Ptr = int(row[6])
                        self.num_charge = int(row[7]) #慢充
                        self.charge_price = int(row[9]) #慢充
                        self.Pchar = int(row[11]) #慢充
        except FileNotFoundError:
            print('文件未被找到')
        except Exception as e:
            print('發生錯誤', e)
    ...
    def schedule(self):
        try:
            ...
            for t in range(now_time-1, num_time): #超過契約容量之罰金
                Pbuy[t] = m.addVar(lb=0)
                pc_cost[t] = m.addVar(lb=0)
                Pnet[t] += ev_load[t]
                for index in range(len(se_list)):
                    Pnet[t] = Pnet[t] + se_char[t][index] * efficiency
                m.addConstr(Pnet[t] <= Pbuy[t])
                total_cost = total_cost + Pbuy[t] * tou[t] / 12 

                m.addConstr(Pnet[t] - Ptr <= pc_cost[t])               
            ...
            #         add_soc = x_se_char[t][index] *efficiency /12 /ev_list[i].capacity
...
#測試用                
# myfcs = road_new_ev(17,23,17,23,49,72,100,1,5.5,7.2)
...
# myfcs_2 = road_new_ev(16,122,16,32,35,83,100,2,3.1,6.8)
