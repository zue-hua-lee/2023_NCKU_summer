...
func (t *ServiceSetup) Match(stationID, chargerID, maxSoC, price, now_offerID string) (string, error) {
	req := channel.Request{ChaincodeID: t.ChaincodeID, Fcn: "match", Args: [][]byte{[]byte(stationID), []byte(chargerID), []byte(maxSoC), []byte(price), []byte(now_offerID)}}
	respone, err := t.Client.Execute(req)
	if err != nil {
		return "", err
	}
